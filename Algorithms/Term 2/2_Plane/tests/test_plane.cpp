...
public:
  UnknownPassenger(const std::vector<int> &handLuggageWeights,
                   const std::vector<int> &luggageWeights)
      : Passenger(handLuggageWeights, luggageWeights) {}

  std::string getType() const override { return "UNKNOWN"; }
...
void testSegmentMethods() {
  ...
  auto passenger = std::make_shared<EconomyPassenger>(std::vector<int>{5},
                                                      std::vector<int>{10});
...
void testSegmentOverflow() {
  ...
  auto passenger1 = std::make_shared<EconomyPassenger>(std::vector<int>{5},
                                                       std::vector<int>{10});
  assert(segment.addUnit(passenger1));

  auto passenger2 = std::make_shared<EconomyPassenger>(std::vector<int>{5},
                                                       std::vector<int>{10});
...
void testRemoveLuggageFromEconomy() {
  ...
  auto passenger = std::make_shared<BusinessPassenger>(std::vector<int>{5},
                                                       std::vector<int>{10});

  assert(plane.addPassenger(passenger));

  // Проверяем, что багаж пассажира был размещён после удаления багажа из
  // эконом-класса
...
void testAddPassengerFailures() {
  ...
  auto passenger7 = std::make_shared<EconomyPassenger>(std::vector<int>{5},
                                                       std::vector<int>{30});
...
void testBaggageNotAccommodated() {
  ...
  auto passenger = std::make_shared<BusinessPassenger>(std::vector<int>{10},
                                                       std::vector<int>{30});
  ...
  assert(businessSegment->getCurrentWeight() ==
         passenger->getTotalHandLuggageWeight());
...
void testUnknownPassenger() {
  ...
  auto passenger = std::make_shared<UnknownPassenger>(std::vector<int>{5},
                                                      std::vector<int>{10});
  assert(!plane.addPassenger(passenger));
...
void testPlane() {
  ...
  auto economyPassenger = std::make_shared<EconomyPassenger>(
      std::vector<int>{5}, std::vector<int>{10});
  assert(plane.addPassenger(economyPassenger));

  auto businessPassenger = std::make_shared<BusinessPassenger>(
      std::vector<int>{5}, std::vector<int>{20});
...
void testExceedHandLuggagePieces() {
  ...
  auto passenger = std::make_shared<EconomyPassenger>(std::vector<int>{5, 5},
                                                      std::vector<int>{10});
  assert(!plane.addPassenger(passenger));
...
void testExceedHandLuggageWeight() {
  ...
  auto passenger = std::make_shared<EconomyPassenger>(std::vector<int>{15},
                                                      std::vector<int>{10});
  assert(!plane.addPassenger(passenger));
...
void testUnknownPassengerType() {
  ...
  public:
    SpecialPassenger(const std::vector<int> &handLuggageWeights,
                     const std::vector<int> &luggageWeights)
        : Passenger(handLuggageWeights, luggageWeights) {}
  ...
  auto passenger = std::make_shared<SpecialPassenger>(std::vector<int>{5},
                                                      std::vector<int>{10});
  assert(!plane.addPassenger(passenger));
