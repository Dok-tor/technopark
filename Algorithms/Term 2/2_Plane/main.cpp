...
int main() {
  ...
  while (std::getline(std::cin, line)) {
    ...
    if (type == "ECONOMY") {
      passenger = std::make_shared<EconomyPassenger>(std::move(handLuggage),
                                                     std::move(luggage));
    } else if (type == "BUSINESS") {
      passenger = std::make_shared<BusinessPassenger>(std::move(handLuggage),
                                                      std::move(luggage));
    } else if (type == "FIRST_CLASS") {
      passenger = std::make_shared<FirstClassPassenger>(std::move(handLuggage),
                                                        std::move(luggage));
    }
    ...
    if (passenger) {
      ...
      // plane.printLoadInfo(); // Отладочный вывод
    }
  }
