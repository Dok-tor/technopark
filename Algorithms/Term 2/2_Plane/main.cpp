...
public:
    ...
    virtual int getMaxHandLuggageWeight() const = 0;
    virtual int getMaxLuggagePieces() const = 0;
    virtual int getMaxLuggageWeightPerPiece() const = 0;
};
...
public:
    ...
    int getMaxHandLuggageWeight() const override {
        return 10;
    }
    ...
    int getMaxLuggageWeightPerPiece() const override {
        return 24;
    }
};
...
private:
    ...
    // Отдельные списки для ручной клади и багажа
    std::vector<int> handLuggageWeights;
...
public:
    ...
    void addHandLuggage(int weight) {
        handLuggageWeights.push_back(weight);
        currentWeight += weight;
    }

    void removeHandLuggage(int weight) {
        auto it = std::find(handLuggageWeights.begin(), handLuggageWeights.end(), weight);
        if (it != handLuggageWeights.end()) {
            handLuggageWeights.erase(it);
            currentWeight -= weight;
        }
    }
    ...
    void addCheckedBaggage(int weight) {
        checkedBaggageWeights.push_back(weight);
        currentWeight += weight;
    }

    void removeCheckedBaggage(int weight) {
...
public:
    ...
    bool removeLuggageFromEconomy(const std::shared_ptr<Segment>& economySegment, int requiredWeight) {
        ...
        if (availableCapacity >= requiredWeight) {
            ...
            for (int weight : baggageToRemove) {
                economySegment->removeCheckedBaggage(weight);
            }
            return true;
        } else {
    ...
    bool addPassenger(const std::shared_ptr<Passenger>& passenger) {
        ...
        for (int weight : passenger->getHandLuggageWeights()) {
            if (weight > passenger->getMaxHandLuggageWeight()) {
                std::cout << "!!CANT REGISTER " << type << " PASSENGER, ID = " << passenger->getId() << "!!\n";
                return false;
            }
        }
        ...
        for (int weight : passenger->getLuggageWeights()) {
            if (weight > passenger->getMaxLuggageWeightPerPiece()) {
                std::cout << "!!CANT REGISTER " << type << " PASSENGER, ID = " << passenger->getId() << "!!\n";
                return false;
            }
        }
        ...
        // Добавляем ручную кладь в свой сегмент
        for (int weight : passenger->getHandLuggageWeights()) {
            segment->addHandLuggage(weight);
        }


        // Обработка багажа
        bool allBaggagePlaced = true;
        for (int baggageWeight : passenger->getLuggageWeights()) {
            // Пытаемся разместить багаж в своём сегменте
            if (segment->canAccommodateLuggage(baggageWeight)) {
                segment->addCheckedBaggage(baggageWeight);
            } else if (type == "FIRST_CLASS" || type == "BUSINESS") {
                // Пытаемся разместить багаж в эконом-классе
                auto economySegment = segments["ECONOMY"];
                if (economySegment->canAccommodateLuggage(baggageWeight)) {
                    economySegment->addCheckedBaggage(baggageWeight);
                } else {
                    // Необходимо снять багаж из эконом-класса
                    if (removeLuggageFromEconomy(economySegment, baggageWeight)) {
                        economySegment->addCheckedBaggage(baggageWeight);
                        std::cout << "!!PASSENGER'S LUGGAGE REMOVED FROM FLIGHT, ID = " << passenger->getId() << "!!\n";
                    } else {
                        // Не удалось освободить место
                        allBaggagePlaced = false;
                        break;
                    }
                }
        ...
        if (!allBaggagePlaced) {
            // Если багаж не удалось разместить, откатываем изменения
            for (int weight : passenger->getHandLuggageWeights()) {
                segment->removeHandLuggage(weight);
            }
