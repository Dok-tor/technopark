...
public:
    ...
    void Next(T value) {
        ...
        Node *c;
        while (tmp) {
            if (equ(tmp->data, value)) {
                if (tmp->right) {
                    Node *current = nextInternal(tmp);
                    std::cout << current->data << std::endl;
                    return;
                }
            } else if (cmp(tmp->data, value)) {
                s.push(tmp);
                tmp = tmp->right;
            }else {
                s.push(tmp);
...
void process() {
    AvlTree<size_t, IntComparator, IntEquivalence> AVL;
    std::string op = "0";
    size_t key = 0;
    ...
    while (std::cin >> op >> key)
    {
        if (op == "insert") {
            ...
            std::cout << (AVL.Has(key) ? "true" : "false") << std::endl;
        } else if (op == "delete") {
            AVL.Delete(key);
        } else if (op == "next") {
            AVL.Next(key);
